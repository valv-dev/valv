...
export const clickhouseFunctions: Record<string, FnDef> = {
  ...
  // Distinct counts.
  uniqExact: { args: [{ kind: "column" }], returns: "number", render: ([c]) => `uniqExact(${c})` },
  uniq: { args: [{ kind: "column" }], returns: "number", render: ([c]) => `uniq(${c})` },

  // Conditional aggregation; the predicate's literals become bound params.
  countIf: { args: [{ kind: "predicate" }], returns: "number", render: ([p]) => `countIf(${p})` },
  sumIf: {
