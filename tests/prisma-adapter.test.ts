...
describe("translateFilter", () => {
  it("EqFilter", () => {
    expect(translateFilter({ type: "eq", field: "status", value: "active" })).toEqual({
      status: "active",
    })
  })

  it("InFilter", () => {
    expect(translateFilter({ type: "in", field: "status", values: ["a", "b"] })).toEqual({
      status: { in: ["a", "b"] },
    })
  })

  it("RangeFilter with gte and lte", () => {
    expect(translateFilter({ type: "range", field: "amount", gte: 10, lte: 100 })).toEqual({
      amount: { gte: 10, lte: 100 },
    })
  })

  it("RangeFilter with only gt", () => {
    expect(translateFilter({ type: "range", field: "amount", gt: 5 })).toEqual({
      amount: { gt: 5 },
    })
  })
  ...
  it("LikeFilter contains", () => {
    expect(
      translateFilter({ type: "like", field: "name", value: "foo", mode: "contains" }),
    ).toEqual({ name: { contains: "foo", mode: "insensitive" } })
  })

  it("LikeFilter startsWith", () => {
    expect(
      translateFilter({ type: "like", field: "name", value: "foo", mode: "startsWith" }),
    ).toEqual({ name: { startsWith: "foo", mode: "insensitive" } })
  })

  it("LikeFilter endsWith", () => {
    expect(
      translateFilter({ type: "like", field: "name", value: "foo", mode: "endsWith" }),
    ).toEqual({ name: { endsWith: "foo", mode: "insensitive" } })
  })

  it("NullFilter isNull: true", () => {
    expect(translateFilter({ type: "null", field: "deleted_at", isNull: true })).toEqual({
      deleted_at: null,
    })
  })

  it("NullFilter isNull: false", () => {
    expect(translateFilter({ type: "null", field: "deleted_at", isNull: false })).toEqual({
      deleted_at: { not: null },
    })
  })
  ...
  it("deeply nested OR inside AND (tenant guard + status OR amount range)", () => {
    expect(
      ...
      AND: [{ tenant_id: "t1" }, { OR: [{ total: { gte: 10000 } }, { status: "pending" }] }],
    })
  })
  ...
  it("not(and(eq, like)) — negated compound filter", () => {
    expect(
      ...
      NOT: {
        AND: [{ status: "cancelled" }, { name: { contains: "test", mode: "insensitive" } }],
      },
    })
  })
})
...
describe("PrismaAdapter.execute", () => {
  function makeAdapter() {
    ...
    return {
      ...
      mocks: { findMany, findFirst, create, updateMany, deleteMany },
    }
  }
  ...
  it("find with pagination → take and skip", async () => {
    ...
    expect(mocks.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 11, skip: 20 }))
  })
  ...
  it("belongsTo included record is nulled out when it fails the relation's row filter", async () => {
    const findManyWithCustomer = vi
      .fn()
      .mockResolvedValue([{ id: "o1", customer: { id: "u1", tenant_id: "wrong-tenant" } }])
    const prisma = {
...
describe("matchesFilter (post-fetch belongsTo enforcement)", () => {
  ...
  it("like: case-insensitive contains/startsWith/endsWith", () => {
    expect(
      matchesFilter(
        { name: "Hello World" },
        { type: "like", field: "name", value: "WORLD", mode: "contains" },
      ),
    ).toBe(true)
    expect(
      matchesFilter(
        { name: "Hello World" },
        { type: "like", field: "name", value: "hello", mode: "startsWith" },
      ),
    ).toBe(true)
    expect(
      matchesFilter(
        { name: "Hello World" },
        { type: "like", field: "name", value: "xyz", mode: "endsWith" },
      ),
    ).toBe(false)
  })
  ...
  it("null: matches null/undefined and its negation", () => {
    expect(
      matchesFilter({ deleted_at: null }, { type: "null", field: "deleted_at", isNull: true }),
    ).toBe(true)
    expect(
      matchesFilter(
        { deleted_at: "2026-01-01" },
        { type: "null", field: "deleted_at", isNull: true },
      ),
    ).toBe(false)
    expect(
      matchesFilter(
        { deleted_at: "2026-01-01" },
        { type: "null", field: "deleted_at", isNull: false },
      ),
    ).toBe(true)
  })
  ...
  it("not: negates the inner filter", () => {
    const f = {
      type: "not" as const,
      filter: { type: "eq" as const, field: "status", value: "draft" },
    }
  ...
  it("nulls out a belongsTo include failing a range policy filter", async () => {
    const adapter = new PrismaAdapter({
      order: {
        findMany: vi.fn().mockResolvedValue([{ id: "o1", customer: { id: "c1", credit: 5000 } }]),
      },
    } as unknown as import("@prisma/client").PrismaClient)
